...
export const addListenersInput = selector => {
    const element = document.querySelector(`${selector}`)

    // Evento blur
    element.addEventListener('blur', validarInput)

    // Evento input
    element.addEventListener('input', validarInput)
}

export const removeListenerInput = selector => {
    const element = document.querySelector(`${selector}`)

    const alertas = ['border-l-4', 'border-orange-500', 'rounded-r-lg', 'focus:border-orange-500']
    const classes = ['border', 'border-transparent', 'focus:border-brown-600', 'rounded-lg']

    // Limpiar alertas
    element.classList.remove(...alertas)
    element.classList.add(...classes)

    if(element.nextElementSibling !== null){
        element.nextElementSibling.remove()
        element.previousElementSibling.classList.remove('text-orange-500')
    }

    element.removeEventListener('blur', validarInput)
    element.removeEventListener('input', validarInput)
}
...
export const removelistenersInputs = selector => {
    ...
    const alertas = ['border-l-4', 'border-orange-500']
    ...
    inputs.forEach(input => {
        ...
        if(input.nextElementSibling !== null){
            ...
            input.previousElementSibling.classList.remove('text-orange-500')
        }
...
function validarInput(e){
    ...
    const estado = [['border', 'border-transparent', 'focus:border-brown-600', 'rounded-lg'], ['border-l-4', 'border-orange-500', 'rounded-r-lg', 'focus:border-orange-500']]
    const success = ['border-l-4', 'border-green-500']
    let clases
    ...
    if (clases.includes('border-orange-500')) {
        ...
        if(e.target.nextElementSibling !== null && !e.target.nextElementSibling.classList.contains('no-blur')) {
            ...
            e.target.previousElementSibling.classList.remove('text-orange-500')

            setTimeout(() => {
                e.target.previousElementSibling.classList.remove('text-green-500')
            }, 3000);
        }
    }
}
...
export const validateForm = (array) => {
    // Clases
    const alert = ['block', 'text-orange-500', 'text-xs', 'mt-2']
    const noValido = ['border-l-4', 'border-orange-500', 'rounded-r-lg', 'focus:border-orange-500']
    const valido = ['border', 'border-transparent', 'focus:border-brown-600', 'rounded-lg']
    ...
    for (let elem of array) {
        ...
        if(elem.classList.contains('required') || elem.classList.contains('radio')){
            ...
            elem.classList.remove(...noValido, ...valido)
            elem.parentElement.classList.remove(...noValido)
            if(elem.nextElementSibling) elem.nextElementSibling.classList.remove(...noValido)
            if(elem.previousElementSibling) elem.previousElementSibling.classList.remove('text-orange-500')
            ...
            if(elem.type == 'radio'){
                ...
                const checked = Object.values(radios).some(radio => radio.checked)

                if(!checked){
                    div.innerHTML = Mensaje2
                    div.classList.add('mt-1')

                    const label = document.querySelector(`label[for="${elem.name}"]`)

                    if(label && !label.parentElement.querySelector('#msg-error')){
                ...
                if(elem.value == ""){
                    if(document.querySelector(`[input="${elem.name}"]`)){
                        ...
                        div.innerHTML = Mensaje1
                        !elem.nextElementSibling && elem.parentElement.appendChild(div)
                        elem.classList.add(...noValido)
                        if(elem.previousElementSibling) elem.previousElementSibling.classList.add('text-orange-500')
                        elem.focus()
                        return false
                    }
...
export const spinnerText = `<div class="sk-circle">
